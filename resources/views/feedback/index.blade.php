        ...
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Your Feedback</h3>
                </div>
                @if($feedback->count() > 0)
                                ...
                                @foreach($feedback as $f)
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-900 dark:text-white">{{ $f->task->title }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center space-x-1">
                                                ...
                                                @endfor
                                                <span class="ml-2 text-sm text-gray-600 dark:text-gray-400">{{ $f->rating }}/5</span>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4">
                                            <div class="text-sm text-gray-900 dark:text-white">{{ Str::limit($f->comment, 100) }}</div>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <a href="{{ route('feedback.edit', $f) }}" class="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300">Edit</a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
                ...
                @else
                    <div class="px-6 py-12 text-center">
                        ...
                        <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">You haven't submitted any feedback yet.</p>
                    </div>
                @endif
            </div>
        </div>
