<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('tasks.show', $task) }}" class="text-white font-bold py-2 px-4 rounded transition-colors"
    ...
    <div class="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-200">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
                <h4 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Feedback ({{ $userFeedback->count() }})</h4>
                        ...
                        @foreach($userFeedback as $feedback)
                            <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                                ...
                                <p class="text-xs text-gray-500 dark:text-gray-400">
                                    {{ is_string($feedback->feedback_date) ? \Carbon\Carbon::parse($feedback->feedback_date)->format('M j, Y \a\t g:i A') : $feedback->feedback_date->format('M j, Y \a\t g:i A') }}
                                </p>
                            </div>
                        @endforeach
                    </div>
                @else
                    <div class="text-center py-10">
                        ...
                        <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">No feedback yet</p>
                    </div>
                @endif
            </div>
        </div>
