<x-app-layout>
    <x-slot name="header">
        ...
        <div class="flex items-center gap-2 sm:gap-3">
            <div class="p-1.5 sm:p-2 rounded-lg sm:rounded-xl bg-brand-orange/15 text-brand-orange-dark dark:bg-orange-900/30 dark:text-brand-peach">
        ...
        <a href="{{ route('tasks.show', $task) }}" class="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-3 sm:px-4 py-2 text-sm sm:text-base bg-white dark:bg-gray-800 border border-brand-teal/40 text-brand-teal font-medium rounded-lg sm:rounded-xl hover:bg-brand-teal/10 dark:hover:bg-gray-700 transition-all duration-200 shadow-sm hover:shadow-md">
    ...
    <div class="min-h-screen bg-white dark:bg-gray-950 transition-colors duration-200">
        <div class="max-w-4xl mx-auto px-3 sm:px-4 lg:px-8 py-4 sm:py-6 lg:py-8 space-y-4 sm:space-y-6">
            <!-- Task Info Card -->
            @php
            ...
            <div class="bg-white/95 dark:bg-gray-900/70 backdrop-blur rounded-xl sm:rounded-2xl shadow-xl border border-brand-peach/60 dark:border-gray-800 p-4 sm:p-6">
                <div class="flex items-start gap-3 sm:gap-4">
                    <div class="flex-shrink-0 p-2 sm:p-3 rounded-xl sm:rounded-2xl bg-brand-teal/15 text-brand-teal-dark">
                        <svg class="w-5 h-5 sm:w-6 sm:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/>
                        </svg>
                    </div>
                    <div class="flex-1 min-w-0">
                        <h3 class="text-base sm:text-lg lg:text-xl font-bold text-gray-900 dark:text-white mb-1.5 sm:mb-2">{{ $task->title }}</h3>
                        <p class="text-xs sm:text-sm text-gray-600 dark:text-gray-400 leading-relaxed">{{ $task->description }}</p>
                        <div class="mt-3 sm:mt-4 flex flex-wrap gap-2 sm:gap-3 text-xs font-semibold text-gray-500 dark:text-gray-400">
            ...
            <div class="bg-white/95 dark:bg-gray-900/80 rounded-xl sm:rounded-2xl shadow-2xl border border-brand-peach/60 dark:border-gray-800 p-4 sm:p-6 lg:p-8 space-y-4 sm:space-y-6">
                <div class="mb-4 sm:mb-6">
                    <h3 class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white mb-1.5 sm:mb-2">Share Your Experience</h3>
                    <p class="text-xs sm:text-sm text-gray-600 dark:text-gray-400">Your feedback helps us improve and deliver better results.</p>
                </div>
                ...
                <form action="{{ route('feedback.store', $task) }}" method="POST" novalidate>
                    ...
                    <div class="mb-6 sm:mb-8">
                        ...
                        <div class="bg-brand-peach/20 dark:bg-gray-900/50 rounded-xl sm:rounded-2xl p-3 sm:p-4 lg:p-6 border border-brand-peach/70 dark:border-gray-800">
                            ...
                            <div class="flex flex-wrap items-center justify-center gap-1.5 sm:gap-2 lg:gap-3 mb-3 sm:mb-4" id="rating-container" role="slider" aria-valuemin="0.5" aria-valuemax="5" aria-valuenow="{{ $initialRating ?? 0 }}" aria-valuetext="{{ $initialRating ? ($ratingLabels[(int) round($initialRating)] ?? '') . ' (' . number_format($initialRating, 1) . ' / 5)' : 'No rating selected' }}" tabindex="0">
                                @for($i = 1; $i <= 5; $i++)
                            ...
                            <div class="text-center pt-3 sm:pt-4 border-t border-brand-peach/60 dark:border-gray-800">
                                <p id="rating-text" class="text-sm sm:text-base lg:text-lg font-semibold text-gray-600 dark:text-gray-400 min-h-[1.5rem] sm:min-h-[2rem] transition-all duration-300 px-2">
                    ...
                    <div class="mb-6 sm:mb-8 space-y-3 sm:space-y-4">
                        ...
                        <div class="grid gap-2 sm:gap-3 sm:grid-cols-2">
                            <div class="rounded-xl sm:rounded-2xl border border-brand-peach/70 bg-brand-peach/20 px-3 sm:px-4 py-2 sm:py-3 text-xs text-brand-orange-dark dark:text-brand-peach-dark">
                                Be specific about the task outcome, highlight blockers, and mention teammates that helped.
                            </div>
                            <div class="rounded-xl sm:rounded-2xl border border-brand-teal/70 bg-brand-teal/10 px-3 sm:px-4 py-2 sm:py-3 text-xs text-brand-teal-dark">
                    ...
                    <div class="flex flex-col sm:flex-row justify-end gap-2 sm:gap-3 pt-4 sm:pt-6 border-t border-gray-200 dark:border-gray-700">
                        <a href="{{ route('tasks.show', $task) }}" class="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 py-3 text-sm font-semibold text-brand-teal bg-white dark:bg-gray-800 border-2 border-brand-teal/40 hover:bg-brand-teal/10 rounded-xl transition-all duration-200 shadow-sm hover:shadow-md">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                            </svg>
                            Cancel
                        </a>
                        <button type="submit" class="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 py-3 text-sm font-semibold text-white rounded-xl transition-all duration-200 shadow-lg hover:shadow-xl transform hover:scale-105 active:scale-95 bg-brand-orange hover:bg-brand-orange-dark focus:outline-none focus:ring-2 focus:ring-brand-teal focus:ring-offset-2 focus:ring-offset-white dark:focus:ring-offset-gray-900">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
                            </svg>
                            Submit Feedback
