    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-14 md:h-16">
            ...
            <div class="flex items-center">
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('dashboard') }}">
                        <img src="{{ asset('images/communitaplogo1.svg') }}" alt="CommuniTAP Logo" class="block h-12 md:h-16 w-auto" />
                    </a>
                </div>
            </div>

            <!-- Centered Navigation Links -->
            <div class="flex-1 hidden lg:flex justify-center pl-4 lg:pl-8">
                <div class="space-x-4 lg:space-x-6 xl:space-x-8 sm:-my-px flex">
                    <x-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')">
                        <svg class="w-4 h-4 mr-1.5 lg:mr-2 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    ...
                    <x-nav-link :href="route('tap-nominations.index')" :active="request()->routeIs('tap-nominations.*')">
                        ...
                        <span class="hidden lg:inline">{{ __('Nominations') }}</span>
                        @if($pendingNominationsCount > 0)
                            <span class="ml-1 lg:ml-2 inline-flex items-center px-1.5 lg:px-2 py-0.5 rounded-full text-xs font-medium" style="background-color: rgba(43, 157, 141, 0.2); color: #2B9D8D;">
                    ...
                    @if(Auth::user()->isAdmin())
                        <x-nav-link :href="route('admin.tap-nominations.task-chain')" :active="request()->routeIs('admin.tap-nominations.task-chain')">
                            <span class="hidden lg:inline">🔗 {{ __('Task Chain') }}</span>
                    ...
                    <x-nav-link :href="route('progress')" :active="request()->routeIs('progress')">
                        ...
                        <span class="hidden lg:inline">{{ __('Progress') }}</span>
                    </x-nav-link>
                    ...
                    <x-nav-link :href="route('rewards.index')" :active="request()->routeIs('rewards.*') || request()->routeIs('rewards.mine')">
                        ...
                        <span class="hidden lg:inline">{{ __('Rewards') }}</span>
                    </x-nav-link>
                    ...
                    <x-nav-link :href="route('feedback.index')" :active="request()->routeIs('feedback.*')">
                        ...
                        <span class="hidden lg:inline">{{ __('Feedbacks') }}</span>
                    </x-nav-link>
                    
                    @if(Auth::user()->isAdmin() && request()->is('admin*'))
                        <x-nav-link :href="route('admin.users.index')" :active="request()->routeIs('admin.users.*')">
                            <svg class="w-4 h-4 mr-1.5 lg:mr-2 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <x-nav-link :href="route('admin.task-submissions.index')" :active="request()->routeIs('admin.task-submissions.*')">
                            ...
                            <span class="hidden lg:inline">{{ __('Submissions') }}</span>
                        </x-nav-link>
                        <x-nav-link :href="route('admin.incident-reports.index')" :active="request()->routeIs('admin.incident-reports.*')">
                            <span class="hidden lg:inline">🚨 {{ __('Incident Reports') }}</span>
                            <span class="lg:hidden">🚨</span>
                        </x-nav-link>
                    @endif
                </div>
            ...
            <div class="flex items-center space-x-2 lg:space-x-4">
                <!-- Settings Dropdown -->
                <div class="hidden lg:flex lg:items-center lg:ms-6 space-x-2">
                    <!-- User Dropdown -->
                    <x-dropdown align="right" width="48">
                        <x-slot name="trigger">
                            <button class="inline-flex items-center px-2 md:px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 dark:text-gray-400 bg-white dark:bg-gray-800 hover:text-gray-700 dark:hover:text-gray-300 focus:outline-none transition ease-in-out duration-150">
                                <!-- User Avatar -->
                                <div class="h-7 w-7 md:h-8 md:w-8 rounded-full flex items-center justify-center mr-1 md:mr-2 overflow-hidden bg-gray-300 dark:bg-gray-600">
                                    @if($profilePictureUrl)
                                        <img src="{{ $profilePictureUrl }}" alt="{{ Auth::user()->firstName }}'s avatar" class="w-full h-full object-cover">
                                    @else
                                        <span class="text-xs md:text-sm font-medium text-gray-700 dark:text-gray-300">{{ substr(Auth::user()->firstName, 0, 1) }}{{ substr(Auth::user()->lastName, 0, 1) }}</span>
                                    @endif
                                </div>
                                <span class="hidden lg:inline">{{ Auth::user()->firstName }}</span>
