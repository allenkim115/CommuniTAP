        ...
        <div class="flex items-center justify-between h-16">
            ...
            <div class="flex items-center space-x-6">
                ...
                <div class="hidden md:flex items-center space-x-6">
                    <a href="{{ route('admin.dashboard') ?? '#' }}" class="text-sm font-semibold text-gray-800 dark:text-gray-200 {{ request()->routeIs('admin.dashboard') ? 'border-b-2 pb-1' : '' }}"
                       @if(request()->routeIs('admin.dashboard')) style="border-color: #2B9D8D;" @endif>Dashboard</a>
                    ...
                    <a href="{{ route('admin.tap-nominations.task-chain') }}" class="text-sm font-semibold text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white {{ request()->routeIs('admin.tap-nominations.task-chain') ? 'border-b-2 pb-1' : '' }}"
                       @if(request()->routeIs('admin.tap-nominations.task-chain')) style="border-color: #2B9D8D;" @endif>🔗 Task Chain</a>
                </div>
            </div>
                        ...
                        <x-slot name="content">
                            <div class="px-4 py-2 flex items-center justify-between">
                                ...
                                <form method="POST" action="{{ route('notifications.mark-all-read') }}">
                                    @csrf
                                    <button type="submit" class="text-xs dark:text-orange-400 hover:underline"
                                            style="color: #F3A261;"
                                            onmouseover="this.style.color='#E8944F';"
                                            onmouseout="this.style.color='#F3A261';">Mark all read</button>
                                </form>
                            </div>
                            <div class="max-h-64 overflow-y-auto">
                                @forelse($adminRecentNotifications as $notification)
                                    <div class="px-4 py-3 border-t border-gray-100 dark:border-gray-700 {{ $notification->status === 'unread' ? 'dark:bg-gray-700' : 'bg-white dark:bg-gray-800' }}"
                                         @if($notification->status === 'unread') style="background-color: rgba(243, 162, 97, 0.1);" @endif>
                            ...
                            <div class="px-4 py-2 border-t border-gray-100 dark:border-gray-700">
                                <a href="{{ route('notifications.index') }}" class="text-sm font-semibold dark:text-orange-400 hover:underline"
                                   style="color: #F3A261;"
                                   onmouseover="this.style.color='#E8944F';"
                                   onmouseout="this.style.color='#F3A261';">View all</a>
                            </div>
                        </x-slot>
                    </x-dropdown>
    ...
    <div x-show="open" class="md:hidden border-t border-gray-100 dark:border-gray-800">
        <div class="px-4 py-3 space-y-2">
            ...
            <div class="space-y-1">
                ...
                <a href="{{ route('admin.task-submissions.history') }}" class="block text-sm text-gray-600 dark:text-gray-300 ml-4">Submission History</a>
            </div>
