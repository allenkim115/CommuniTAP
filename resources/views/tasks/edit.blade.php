    ...
    <div class="min-h-screen bg-gradient-to-b from-gray-50 via-orange-50/30 to-teal-50/20 dark:from-gray-900 dark:via-gray-900 dark:to-gray-950 py-10">
        <div class="max-w-6xl mx-auto px-6 lg:px-8">
            <div
                ...
                <!-- Form Section -->
                <form action="{{ route('tasks.update', $task) }}" method="POST" class="space-y-8" novalidate>
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        ...
                        <div class="bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900/50 dark:to-gray-800/50 rounded-xl p-6 border-2 border-gray-200 dark:border-gray-700 space-y-6 shadow-sm">
                            ...
                            <div>
                                ...
                                @php
                                    $sitios = ['Pig Vendor','Ermita Proper','Kastilaan','Sitio Bato','YHC','Eyeseekers','Panagdait','Kawit'];
                                    $currentLocation = old('location', $task->location);
                                    $isSitio = in_array($currentLocation, $sitios);
                                    $locationType = old('location_type', $isSitio ? 'sitio' : 'full_address');
                                @endphp
                                ...
                                <div class="flex gap-4 mb-3">
                                    ...
                                    <label class="flex items-center">
                                        <input type="radio" name="location_type" value="full_address" id="location_type_full_address"
                                            class="mr-2"
                                            {{ $locationType === 'full_address' ? 'checked' : '' }}
                                            onchange="toggleLocationInput()">
                                ...
                                <div id="location_sitio_container">
                                    <select {{ $locationType === 'sitio' ? 'name="location"' : '' }} id="location_sitio"
                                        class="w-full px-4 py-2 border-2 border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-orange-500 dark:bg-gray-800 dark:text-white transition-all"
                                        {{ $locationType === 'sitio' ? 'required' : 'style="display:none;"' }}>
                                        <option value="" disabled {{ !$isSitio ? 'selected' : '' }} hidden>Select a sitio</option>
                                        @foreach($sitios as $loc)
                                ...
                                <div id="location_address_container">
                                    <input type="text" {{ $locationType === 'full_address' ? 'name="location"' : '' }} id="location_address" 
                                        value="{{ $isSitio ? '' : $currentLocation }}"
                                        class="w-full px-4 py-2 border-2 border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-orange-500 dark:bg-gray-800 dark:text-white transition-all"
                                        placeholder="Enter full address (e.g., Street, Barangay, City)"
                                        maxlength="255"
                                        {{ $locationType === 'full_address' ? 'required' : 'style="display:none;"' }}>
            ...
            if (sitioRadio && fullAddressRadio && sitioSelect && addressInput) {
                if (sitioRadio.checked) {
                    ...
                    if (!sitioSelect.value) {
                        addressInput.value = ''; // Clear address when switching to sitio if no sitio selected
                    }
                } else if (fullAddressRadio.checked) {
                    sitioSelect.style.display = 'none';
                    sitioSelect.required = false;
                    sitioSelect.removeAttribute('name');
                    if (sitioSelect.value && !addressInput.value) {
                        sitioSelect.value = ''; // Clear sitio when switching to address if no address entered
                    }
