                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="lg:col-span-2">
                                ...
                                <textarea name="description" id="description" rows="10" maxlength="1000"
                                    class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white"
                                    placeholder="Provide detailed description of the task, including objectives, requirements, and expected outcomes"
                                    required>{{ old('description') }}</textarea>
                                <div class="mt-1 flex items-center justify-between">
                                    <p class="text-xs text-gray-500 dark:text-gray-400">Be specific about objectives,
                                        requirements, and expected outcomes.</p>
                                    <p id="description-counter" class="text-xs text-gray-500 dark:text-gray-400">0/1000
                                    </p>
                                </div>
                                @error('description')
                                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <input type="number" name="points_awarded" id="points_awarded"
                                    value="{{ old('points_awarded') }}"
                                    class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white"
                                    placeholder="Enter points (minimum 1)" min="1" required>
                                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Recommended: 5–100 points
                                    depending on effort.</p>
                            ...
                            <div>
                                ...
                                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Must be at least one day from
                                    today.</p>
                                @error('due_date')
                                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <input type="time" name="end_time" id="end_time" value="{{ old('end_time') }}"
                                        class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white"
                                        required>
                                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Ensure end time is after
                                        start time.</p>
                                    @error('end_time')
                                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                    @enderror
                            ...
                            <div>
                                ...
                                <select name="location" id="location"
                                    ...
                                    <option value="Pig Vendor" {{ old('location') == 'Pig Vendor' ? 'selected' : '' }}>
                                        Pig Vendor</option>
                                    <option value="Ermita Proper"
                                        {{ old('location') == 'Ermita Proper' ? 'selected' : '' }}>Ermita Proper
                                    </option>
                                    <option value="Kastilaan" {{ old('location') == 'Kastilaan' ? 'selected' : '' }}>
                                        Kastilaan</option>
                                    <option value="Sitio Bato" {{ old('location') == 'Sitio Bato' ? 'selected' : '' }}>
                                        Sitio Bato</option>
                                    <option value="YHC" {{ old('location') == 'YHC' ? 'selected' : '' }}>YHC
                                    </option>
                                    <option value="Eyeseekers"
                                        {{ old('location') == 'Eyeseekers' ? 'selected' : '' }}>Eyeseekers</option>
                                    <option value="Panagdait" {{ old('location') == 'Panagdait' ? 'selected' : '' }}>
                                        Panagdait</option>
                                    <option value="Kawit" {{ old('location') == 'Kawit' ? 'selected' : '' }}>Kawit
                                    </option>
                                </select>
                                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Select the sitio for this
                                    task.</p>
                                @error('location')
                                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                <label
                                    class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Participant
                                    Limit</label>
                                <p class="text-sm text-gray-500 dark:text-gray-400 mb-2">Leave blank for unlimited
                                    participants.</p>
